...
class Api
{
    ...
    /**
     * @var \App\Helpers\Geo\Response|null
     */
    protected $_response;
    ...
    public function load(array $options = [])
    {
        ...
        if (curl_errno($curl)) {
            ...
            curl_close($curl);
            throw new \App\Helpers\Geo\Exception\CurlError($error);
        }
        curl_close($curl);
        if (in_array($code, array(500, 502))) {
            $msg = strip_tags($data);
            throw new \App\Helpers\Geo\Exception\ServerError(trim($msg), $code);
        }
...
//        var_dump($data);
        ...
        if (!empty($data['error'])) {
            throw new \App\Helpers\Geo\Exception\MapsError($data['error']['message'], $data['error']['code']);
        }

        $this->_response = new \App\Helpers\Geo\Response($data);
